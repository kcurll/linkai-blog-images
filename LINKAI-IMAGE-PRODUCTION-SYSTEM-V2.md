# LINKAI IMAGE PRODUCTION SYSTEM v2

This document defines the production prompt architecture for the Linkai Digital blog image series.

## Priority Order
1. Subject
2. Composition
3. Style
4. Quality

## Subject
Construct the scene only from the specified Primary, Secondary and Supporting objects. Do not substitute objects.

## Composition
Follow the specified composition exactly.

## Style
Use the approved reference image only for background, glow, lighting, materials, camera angle, colour palette, shadows and rendering quality. Do not copy the subject matter.

## Locked Style
- Bright purple background
- Soft centred glow
- Slight top-down camera
- Premium matte CGI
- 1200x630
- No text, numbers, logos or watermarks
- Green=success, Red=problem, Purple=automation

## Forbidden substitutions
Unless explicitly requested, never substitute: clipboard, checklist, browser window, dashboard, folder, notebook, laptop, phone, calendar, document, paper, monitor or screen.

## QC
Verify required objects, composition, style consistency and absence of text before accepting a render.